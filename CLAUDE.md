...
**ripasso_mate** is a study progress tracker for Analysis 1 (Analisi 1) mathematics. It is a React/TypeScript SPA; `package.json`, `src/`, and all config files live at the repository root.
...
### Routing & Guards ([src/App.tsx](src/App.tsx))
...
### Data ([src/data/topics.ts](src/data/topics.ts))
...
### Study Plan Logic ([src/pages/Plan.tsx](src/pages/Plan.tsx))
...
### Dark Mode ([src/store/useTheme.ts](src/store/useTheme.ts))
...
### Key Tailwind Notes
- Dark mode is `class`-based (configured in [tailwind.config.js](tailwind.config.js)), not `media`-based
