...
import { useCourse, COURSES, type CourseId } from '../store/CourseContext';
...
function CourseDropdown() {
  ...
  function handleSelect(id: CourseId) {
    setCourse(id);
    setOpen(false);
    navigate('/');
  }
  ...
  return (
    <div className="relative">
      <button
        ...
        aria-expanded={open}
      >
        <span className="text-gray-500">{course.icon}</span>
        <div className="flex-1 min-w-0">
          <span className="text-base font-bold text-gray-900 leading-tight block">{course.label}</span>
          <span className="text-xs text-gray-400 block">{course.subtitle}</span>
        </div>
        <svg
          ...
          >
            {COURSES.map(c => (
              <li key={c.id}>
                ...
                >
                  <span className={c.id === course.id ? 'text-amber-600' : 'text-gray-400'}>
                    {c.icon}
                  </span>
